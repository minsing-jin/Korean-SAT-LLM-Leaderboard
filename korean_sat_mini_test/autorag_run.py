...
async def async_kice_metric(
    ...
    completion = await client.beta.chat.completions.parse(
        ...
        messages=[
            {"role": "system",
             "content": "당신은 시험을 채점하는 채점관입니다. 학생의 대답을 보고, 학생이 몇 번을 선택하였는지 구분하세요. 학생이 선택한 답변을 반환하세요. 만약 학생이 답변을 하지 못했다면 0번을 반환하세요."},
            {"role": "user",
             "content": "해당 문제는 동건이의 콧수염에 대하여 묻는 문제입니다. 동건이는 콧수염이 있지만, 그 길이가 예전에 비해 길지 않으므로 가장 적합한 선택지는 3번입니다. 형식 또한 Answer: 3이므로 3번입니다."},
...
@click.option('--config', type=click.Path(exists=True, dir_okay=False), help='Path to config file',
              default=os.path.join('./korean_sat_mini_test', 'autorag_config.yaml'))
